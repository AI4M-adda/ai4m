...
export default function GalexyCanvas() {
  useEffect(() => {
    ...
    function update() {
      ...
      for (let i = 0; i < CHARS.length; i++) {
        const DX = 0.0 * Math.sin(time * 0.001);
    ...
    function createChars() {
      for (let i = 0; i < MAX_CHARS; i++) {
        const CHARACTER = "."; //String.fromCharCode((Math.random() * 10 + 34) | 0);
  ...
  return (
    <div
      className={cn(
        "w-full h-full min-h-[200px] min-w-[200px] overflow-hidden"
      )}
    >
      <canvas id="galaxy-bg" style={{ width: "100%", height: "100%" }} />
    </div>
  );
}
